...
export const AdminChart = () => {
  ...
  useEffect(() => {
    ...
    const analyticsData = analyticsService.getAdminAnalytics();
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-6">
          ...
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-white">
            <span className="bg-gradient-to-r from-ruby-400 to-ruby-600 bg-clip-text text-transparent">Admin Analytics</span>
          </h1>
          <p className="text-gray-400 mt-2 text-sm sm:text-base">Track total number of users in the system over time</p>
        </div>
          ...
          <>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 mb-6">
              <div className="bg-gray-800/90 backdrop-blur-sm p-4 sm:p-6 rounded-xl shadow-md hover:shadow-ruby-glow transition-all border-2 border-ruby-600/30">
                <h3 className="text-xs sm:text-sm md:text-base font-medium text-gray-300 mb-2">Current Users</h3>
                <p className="text-2xl sm:text-3xl md:text-4xl font-bold text-ruby-400">{stats.current}</p>
                <p className="text-xs sm:text-sm text-gray-400 mt-1">Total users in system</p>
              </div>
              <div className="bg-gray-800/90 backdrop-blur-sm p-4 sm:p-6 rounded-xl shadow-md hover:shadow-ruby-glow transition-all border-2 border-ruby-600/30">
                <h3 className="text-xs sm:text-sm md:text-base font-medium text-gray-300 mb-2">Previous Day</h3>
                <p className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-300">{stats.previous}</p>
                <p className="text-xs sm:text-sm text-gray-400 mt-1">Users yesterday</p>
              </div>
              <div className="bg-gray-800/90 backdrop-blur-sm p-4 sm:p-6 rounded-xl shadow-md hover:shadow-ruby-glow transition-all border-2 border-ruby-600/30">
                ...
                <p className={`text-xl sm:text-2xl lg:text-3xl font-bold ${stats.change >= 0 ? 'text-ruby-400' : 'text-red-400'}`}>
                  {stats.change >= 0 ? '+' : ''}{stats.change.toFixed(0)} ({stats.changePercent >= 0 ? '+' : ''}{stats.changePercent.toFixed(1)}%)
                </p>
                <p className="text-xs sm:text-sm text-gray-400 mt-1">Since yesterday</p>
              </div>
            </div>

            <div className="bg-gray-800/90 backdrop-blur-sm p-4 sm:p-6 rounded-xl shadow-lg border-2 border-ruby-600/30">
              <h2 className="text-base sm:text-lg md:text-xl font-bold mb-2 sm:mb-3 md:mb-4 text-white">Total Users Trend (Last 30 Days)</h2>
              <p className="text-xs sm:text-sm text-gray-400 mb-3 sm:mb-4">Shows the actual number of users in the system. Ups and downs reflect when users are added or removed.</p>
