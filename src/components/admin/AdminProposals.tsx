      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-6 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-white">
              <span className="bg-gradient-to-r from-ruby-400 to-ruby-600 bg-clip-text text-transparent">All Capstone Proposals</span>
            </h1>
            <p className="text-gray-400 mt-2 text-sm sm:text-base">View all student capstone proposals with PDFs and their history</p>
          </div>
          <select
            value={filterStatus}
            ...
            <div key={group.id} className="bg-gray-800/90 backdrop-blur-sm shadow-lg rounded-xl p-4 sm:p-6 border-2 border-ruby-600/30">
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-4 gap-2">
                <div className="flex-1">
                  ...
                  <p className="text-xs sm:text-sm text-gray-400">
                    Created by: <strong className="text-ruby-400">{getUserName(group.createdBy)}</strong> on {new Date(group.createdAt).toLocaleDateString()}
                  </p>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
                ...
                {group.teachers && group.teachers.length > 0 && (
                  <div>
                    <p className="text-sm sm:text-base font-medium text-gray-300 mb-1">Assigned Teachers:</p>
              ...
              {group.files && group.files.length > 0 && (
                <div className="mt-4 border-t-2 border-ruby-600/30 pt-4">
                  <h4 className="text-sm sm:text-base font-bold text-white mb-3">Proposal PDF Files:</h4>
        ...
        {filteredGroups.length === 0 && (
          <div className="text-center py-12">
            <p className="text-gray-400 text-sm sm:text-base">No proposals found.</p>
          </div>
        )}

        <div className="mt-4 text-xs sm:text-sm text-gray-400">
          Total Proposals: <span className="font-bold text-ruby-400">{filteredGroups.length}</span>
